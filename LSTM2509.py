...
for i in range(len(dl)):
    ...
    l2=[]
    step=28
    t=[dl[i][j:j+step] for j in range(0,len(dl[i])) if len(dl[i][j:j+step])==28]
    l=[list(map(itemgetter(0),i)) for i in t]
    for h in range(len(t)-28):
        dataall.append([m for m in t[h]])
        label.append(max(l[h+28]))
        t2.append([m for m in t[h]])
        l2.append(max(l[h+28]))
    dt.append(t2)
    lt.append(l2)
...
for i in range(len(dl2)):
    ...
    l2=[]
    step=28
    t=[dl2[i][j:j+step] for j in range(0,len(dl2[i])) if len(dl2[i][j:j+step])==28]
    l=[list(map(itemgetter(0),i)) for i in t]
    for h in range(len(t)-28):
        dataall2.append([m for m in t[h]])
        label2.append(max(l[h+28]))
        t2.append([m for m in t[h]])
        l2.append(max(l[h+28]))
    dt2.append(t2)
    lt2.append(l2)
...
model = Sequential()
model.add(LSTM(500, activation='relu',input_shape=(train_X.shape[1], train_X.shape[2])))
